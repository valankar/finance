...
TWO_MONTHS_AGO_FLOOR = (TODAY_DATE +
...
def make_assets_breakdown_section(daily_df):
    ...
    columns = [
        'total', 'total_real_estate', 'total_no_homes', 'total_retirement',
        'total_investing', 'total_liquid'
    ]
...
def get_investing_retirement_df(daily_df, accounts_df):
    ...
    ibonds_df = accounts_df['USD']['Treasury Direct'].droplevel(
        1, 1).rename(columns={
            'nan': 'ibonds'
        }).fillna(0)
    merged_df = pd.merge_asof(invret_df,
                              ibonds_df,
                              left_index=True,
                              right_index=True)
    pal_df = accounts_df['USD']['Bank 2']['Liability 1'].rename(
        columns={
            'nan': 'liability_1'
        }).fillna(0)
...
def make_real_estate_section(real_estate_df):
    ...
    # Profit bars
    section.add_trace(make_real_estate_profit_bar(real_estate_df),
                      row=1,
                      col=2)
    section.add_trace(make_real_estate_profit_bar_yearly(real_estate_df),
                      row=2,
                      col=2)
    ...
    section.update_layout(title_text='Real Estate',
                          legend=dict(
                              yanchor='top',
                              y=1,
                              xanchor='left',
...
def make_real_estate_profit_bar_yearly(real_estate_df):
    ...
    percent = []
    for home in HOMES:
        time_diff = real_estate_df[home].index[-1] - \
...
def make_profit_bar(invret_df):
    ...
    with open(common.PREFIX + 'commodities_cost_basis.txt',
              encoding='utf-8') as commodities_file:
        commodities_cost_basis = float(commodities_file.read())
    commodities_market_value = invret_df[['commodities'
                                          ]].iloc[-1]['commodities']
    commodities_profit = commodities_market_value - commodities_cost_basis
    with open(common.PREFIX + 'etfs_cost_basis.txt',
              encoding='utf-8') as etfs_file:
    ...
    with open(common.PREFIX + 'treasury_direct_cost_basis.txt',
              encoding='utf-8') as ibonds_file:
    ...
    profit_bar = go.Figure(
        go.Bar(
            ...
            y=values,
            marker_color=[
                COLOR_GREEN if x >= 0 else COLOR_RED for x in values
            ],
            text=[
                f'{Float(x):.2h}<br>{y:.2f}%' for x, y in zip(values, percent)
            ],
        ))
    return profit_bar
...
def make_allocation_profit_section(daily_df, invret_df):
    ...
    changes_section.update_traces(row=1,
                                  col=1,
...
def make_change_section(daily_df, column, title):
    ...
    changes_section = make_subplots(
        rows=2,
        cols=2,
        subplot_titles=('Year Over Year', 'Month Over Month', 'Week Over Week',
                        'Day Over Day'),
        vertical_spacing=0.07,
        horizontal_spacing=0.05,
    )
    for trace in make_total_bar_yoy(daily_df, column).data:
        trace.marker.color = [
            COLOR_GREEN if y > 0 else COLOR_RED for y in trace.y
        ]
        changes_section.add_trace(trace, row=1, col=1)
...
def make_total_bar_wow(daily_df, column):
    ...
    diff_df = daily_df.resample(
        'W').last().interpolate().diff().dropna().iloc[-52:]
...
def make_total_bar_mom(daily_df, column):
    ...
    diff_df = daily_df.resample(
        'M').last().interpolate().diff().dropna().iloc[-36:]
...
def append_day_difference_table(all_df, accounts_df, output_file):
    ...
    accounts_df = accounts_df.iloc[-1] - accounts_df.iloc[
        accounts_df.index.get_indexer([YESTERDAY], method='nearest')]
    accounts_df = accounts_df.loc[:, accounts_df.sum() != 0]
    daily_df = all_df.iloc[-1] - all_df.iloc[all_df.index.get_indexer(
        [YESTERDAY], method='nearest')]
...
def write_dynamic_plots(all_df, accounts_df, section_tuples):
    ...
    with common.temporary_file_move(
            f'{common.PREFIX}{INDEX_HTML}') as index_file:
        for section, height, width in section_tuples:
            ...
            index_file.write(
                section.to_html(full_html=False,
...
def write_static_plots(all_df, accounts_df, section_tuples):
    ...
    image_name = 1
    with common.temporary_file_move(
            f'{common.PREFIX}{STATIC_HTML}') as index_file:
        for section, height, width in section_tuples:
            section.write_image(common.PREFIX + f'images/{image_name}.png',
...
def get_real_estate_df(accounts_df):
    ...
    for home in HOMES:
        real_estate_df[f'{home} Percent Change'] = (
            (real_estate_df[home] -
             real_estate_df.loc[real_estate_df[home].first_valid_index(),
                                home]) /
            real_estate_df.loc[real_estate_df[home].first_valid_index(),
                               home] * 100)
...
def main():
    ...
    accounts_df = pd.read_csv(
        ...
        parse_dates=True,
        infer_datetime_format=True,
        header=[0, 1, 2, 3]).tz_localize('UTC').tz_convert(TIMEZONE)
    ...
    invret_section = make_investing_retirement_section(
        downsample_df(invret_df))
    real_estate_section = make_real_estate_section(
        downsample_df(real_estate_daily_df))
    allocation_section = make_allocation_profit_section(daily_df, invret_df)
    net_worth_change_section = make_change_section(daily_df, 'total',
    ...
    write_html_and_images(
        (
            (assets_trend, 1, 1),
            (invret_section, 1, 1),
            (real_estate_section, .75, 1),
            (allocation_section, .5, 1),
