...
def main():
    ...
    account_map = {
        ...
        ('USD', 'Vanguard 401k', 'Vanguard Target Retirement 2040 Trust'),
    ...
    accounts_df = pd.read_csv(common.PREFIX + 'account_history.csv',
                              index_col=0,
                              parse_dates=True,
    ...
    commodities = 0.0
    with open(common.PREFIX + 'commodities_values.csv',
              newline='',
    ...
    etfs = 0.0
    with open(common.PREFIX + 'etfs_values.csv', newline='',
              encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            value = float(row['value'])
    ...
    with open(common.PREFIX + 'treasury_direct.txt',
    ...
    for chf_account in ['bank_chf', 'bank_chf_visa', 'cash_chf', 'bank5_chf']:
        with open(common.PREFIX + chf_account + '.txt',
                  encoding='utf-8') as input_file:
    ...
    for usd_account in [
            'bank5_usd', 'bank7', 'bank1', 'bank1_liability2',
            'bank1_liability3', 'brokerage_brokerage_cash',
            'brokerage_checking', 'brokerage_liability_1', 'bank4_cash'
    ]:
        with open(common.PREFIX + usd_account + '.txt',
                  encoding='utf-8') as input_file:
            value = float(input_file.read())
            total_liquid += value
    ...
    for estate in ['property3', 'property4', 'property5']:
        with open(common.PREFIX + estate + '.txt',
                  encoding='utf-8') as input_file:
            value = float(input_file.read())
    ...
    with open(common.PREFIX + '401k_values.csv', newline='',
              encoding='utf-8') as csvfile:
        vanguard = 0.0
        reader = csv.DictReader(csvfile)
        for row in reader:
    ...
    with open(common.PREFIX + 'bank_chf_pillar2.txt',
              encoding='utf-8') as input_file:
        value = float(input_file.read())
    ...
    fieldnames = [
        'total', 'total_no_homes', 'total_liquid', 'total_real_estate',
        'total_retirement', 'total_investing', 'etfs', 'commodities', '401k',
        'pillar2'
    ]
    data = {
        'total': total,
    ...
    history_df.to_csv(common.PREFIX + 'history.csv',
                      mode='a',
                      header=False,
                      float_format='%.2f')
    accounts_df[-1:].to_csv(common.PREFIX + 'account_history.csv',
                            mode='a',
                            header=False,
                            float_format='%.2f')
